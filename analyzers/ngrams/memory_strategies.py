...
class ExternalSortUniqueExtractor:
    ...
    def __init__(self, memory_manager: MemoryManager, temp_dir: Optional[str] = None):
        ...
        self.temp_files = []
        self.logger = logging.getLogger("external_sort")
        
    def extract_unique(self, ldf_data: pl.LazyFrame, column_name: str = "ngram_text") -> pl.DataFrame:
    ...
    def _create_sorted_chunks(self, ldf_data: pl.LazyFrame, column_name: str) -> List[str]:
        """Create sorted temporary files from input data."""
        chunk_files = []
        
        # Use very small chunks for critical memory pressure
        chunk_size = self.memory_manager.calculate_adaptive_chunk_size(10000, "unique_extraction")
        
        total_count = ldf_data.select(pl.len()).collect().item()
        total_chunks = (total_count + chunk_size - 1) // chunk_size
        
        self.logger.info(f"Creating {total_chunks} sorted chunks with chunk size {chunk_size}")
        ...
        for chunk_idx in range(total_chunks):
            ...
            try:
                ...
                chunk_file = os.path.join(self.temp_dir, f"ngram_chunk_{chunk_idx}.parquet")
                ...
                self.logger.warning(f"Failed to process chunk {chunk_idx}: {e}")
                continue
                
        return chunk_files
    
    def _merge_sorted_chunks(self, chunk_files: List[str], column_name: str) -> pl.DataFrame:
        ...
        for i, chunk_file in enumerate(chunk_files):
            try:
                ...
                self.logger.warning(f"Failed to read chunk file {chunk_file}: {e}")
                continue
