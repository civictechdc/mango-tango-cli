...
class MemoryAwareProgressManager(RichProgressManager):
    ...
    def update_step_with_memory(self, step_id: str, current: int, 
    ...
    def _display_memory_warning(self, pressure_level: MemoryPressureLevel, 
                               memory_stats: Dict, context: str) -> None:
        """Display memory pressure warning to user."""
        # Avoid spam - only show warning every 30 seconds
        current_time = time.time()
        if (self.last_memory_warning and 
            current_time - self.last_memory_warning < 30):
            return
        ...
        warning_text.append(f"({memory_stats['process_memory_percent']:.1f}% of limit)", style=pressure_color)
        ...
        if pressure_level == MemoryPressureLevel.CRITICAL:
            warning_text.append("\n⚠️  Critical memory pressure - switching to disk-based processing", style="red bold")
