...
class PresenterView(MethodView):
    ...
    def get(self, id: str):
        ...
        return (
            (jsonify({"code": 200, "data": output}), 200)
            if not output is None
            else (jsonify({"code": 404, "error": "Presenter not found"}), 404)
        )
...
class PresenterDownloadView(MethodView):
    ...
    def get(self, id: str, file_type: str):
        ...
        columns = [
            column
            for column in analyzer_output["columns"]
            if not column["api_field"] is None
        ]
        data_length = 0
        ...
        if len(columns) > 0:
            column = columns[0]
            data_length = (
                len(output[column["api_field"]][column["dict_field"]])
        ...
        for index in range(0, data_length):
            ...
            for column in columns:
                item_key = (
                    column["api_name"]
                    if not column.get("api_name", None) is None
                    else column["api_field"]
                )
                ...
                if column["data_type"] == "text" or column["data_type"] == "identifier":
                    item_value = (
                        column_data[column["dict_field"]][index]
                        if type(column_data) is dict
                        else column_data[index]
                    )
        ...
        if file_type.lower() == FileDownloadChoiceEnum.Excel.value:
            ...
            workbook = Workbook(buffer, {"remove_timezone": True})
