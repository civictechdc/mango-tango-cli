...
class TestProgressManager(unittest.TestCase):
    """Test suite for the new ProgressManager with Textual + Rich hybrid approach."""
    ...
    def test_enhanced_60fps_refresh_rate(self):
        """Test that ProgressManager uses enhanced 60fps refresh rate."""
        self.progress_manager.add_step("step1", "Test Step")

        # Start the progress manager and verify refresh rate
        with self.progress_manager:
            if self.progress_manager.live:
                # Verify enhanced refresh rate (60fps vs RichProgressManager's 4fps)
                self.assertEqual(self.progress_manager.live.refresh_per_second, 60)

    def test_api_compatibility_with_rich_progress_manager(self):
        """Test that ProgressManager maintains API compatibility with RichProgressManager."""
    ...
    def test_table_rebuild_functionality(self):
        """Test that table rebuilding works correctly with positional insertion."""
        self.progress_manager.add_step("step1", "First Step")
        self.progress_manager.add_step("step2", "Second Step")

        # Rebuild table and verify structure
        self.progress_manager._rebuild_table()

        # Verify table has correct number of rows
        # (Note: This is a basic test since Rich Table doesn't expose row count directly)
        self.assertIsNotNone(self.progress_manager.table)

        # Add substeps and rebuild
...
class TestProgressManagerPositionalInsertion(unittest.TestCase):
    """Dedicated test class for positional insertion edge cases and advanced scenarios."""
