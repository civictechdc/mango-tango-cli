...
class RichProgressManager:
    ...
    def _update_display(self):
        """Update the Rich display with current step states and active progress."""
        with self._display_lock:
            ...
            # Create the main table for all steps (always show all steps)
            steps_table = Table(
            ...
            # Add each step to the table - ALL steps are shown from the beginning
            ...
            # Add active progress bar if there's an active step with total
            if (
                self.active_step
                ...
                if step_info["total"] is not None:
                    content_parts.append("")  # Empty line
                    # Add the Rich progress display for the active task
                    content_parts.append(self.progress)

            # Update the display group and live display
            # Create a new Group with the updated content
            from rich.console import Group

            self.display_group = Group(*content_parts)
            self.live.update(self.display_group)
            # Rich Live will automatically refresh based on auto_refresh=True
